...
extern "C"
{
		...
		FillDATPluginInfo(DAT_PluginInfo* info)
	{
		...
		// or numbers (a-z, 0-9)
		info->customOPInfo.opType->setString("Mocapsync");

		// The opLabel is the text that will show up in the OP Create Dialog
		info->customOPInfo.opLabel->setString("MoCap Sync Cameras");

		// Will be turned into a 3 letter icon on the nodes
		info->customOPInfo.opIcon->setString("MCS");
		...
		info->customOPInfo.maxInputs = 1024;
	}
		...
		CreateDATInstance(const OP_NodeInfo* info)
	{
		...
		return new TD_MoCap::OP_SyncCameras(info);
	}
		...
		DestroyDATInstance(DAT_CPlusPlusBase* instance)
	{
		...
		delete (TD_MoCap::OP_SyncCameras*)instance;
	}
