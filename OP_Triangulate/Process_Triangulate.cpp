...
#pragma mark Process_Triangulate
			...
			, TriangulateParameters& parameters)
	{
		...
		if (parameters.cameraLeft.empty()) {
			throw(Exception("No intrinsics loaded for camera left"));
		}
		if (parameters.cameraRight.empty()) {
			throw(Exception("No intrinsics loaded for camera left"));
		}
		if (parameters.fundamental_matrix.empty()) {
			throw(Exception("No stereo calibration loaded"));
		}
		...
		// Check empty frame
		if (!leftCamera->centroids.empty() && !rightCamera->centroids.empty()) {
			// Undistort centroids
