...
extern "C"
{
		...
		FillDATPluginInfo(DAT_PluginInfo* info)
	{
		...
		// or numbers (a-z, 0-9)
		info->customOPInfo.opType->setString("Filterandbin");

		// The opLabel is the text that will show up in the OP Create Dialog
		info->customOPInfo.opLabel->setString("MoCap Filter and Bin");

		// Will be turned into a 3 letter icon on the nodes
		info->customOPInfo.opIcon->setString("FAB");
		...
		CreateDATInstance(const OP_NodeInfo* info)
	{
		...
		return new TD_MoCap::ThreadedOP<TD_MoCap::Process_FilterAndBin>(info);
	}
		...
		DestroyDATInstance(DAT_CPlusPlusBase* instance)
	{
		...
		delete (TD_MoCap::ThreadedOP<TD_MoCap::Process_FilterAndBin>*)instance;
	}
