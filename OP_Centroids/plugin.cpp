...
extern "C"
{
		...
		FillDATPluginInfo(DAT_PluginInfo* info)
	{
		...
		// or numbers (a-z, 0-9)
		info->customOPInfo.opType->setString("Centroids");

		// The opLabel is the text that will show up in the OP Create Dialog
		info->customOPInfo.opLabel->setString("MoCap Centroids");

		// Will be turned into a 3 letter icon on the nodes
		info->customOPInfo.opIcon->setString("CEN");
		...
		CreateDATInstance(const OP_NodeInfo* info)
	{
		...
		return new TD_MoCap::ThreadedOP<TD_MoCap::Process_Centroids>(info);
	}
		...
		DestroyDATInstance(DAT_CPlusPlusBase* instance)
	{
		...
		delete (TD_MoCap::ThreadedOP<TD_MoCap::Process_Centroids>*)instance;
	}
