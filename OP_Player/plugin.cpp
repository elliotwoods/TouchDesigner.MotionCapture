...
extern "C"
{
		...
		FillDATPluginInfo(DAT_PluginInfo* info)
	{
		...
		// or numbers (a-z, 0-9)
		info->customOPInfo.opType->setString("Mocapplayer");

		// The opLabel is the text that will show up in the OP Create Dialog
		info->customOPInfo.opLabel->setString("MoCap Player");

		// Will be turned into a 3 letter icon on the nodes
		info->customOPInfo.opIcon->setString("MCP");
		...
		CreateDATInstance(const OP_NodeInfo* info)
	{
		...
		return new TD_MoCap::OP_Player(info);
	}
		...
		DestroyDATInstance(DAT_CPlusPlusBase* instance)
	{
		...
		delete (TD_MoCap::OP_Player*)instance;
	}
